        ...
        <div
          className={`relative overflow-hidden bg-muted ${
            isCircular
              ...
              : "aspect-video w-full max-w-2xl rounded-lg"
          }`}
        >
          <Image
            src={currentImageUrl}
...
export function BlogPostForm({ initialData, onSubmit }: BlogPostFormProps) {
  ...
  const handleImageUpload = async (
    file: File,
    fieldName: "imageUrl" | "authorProfileImage"
  ) => {
    console.log(
      "handleImageUpload called with:",
      file.name,
      file.type,
      file.size,
      "for field:",
    ...
    const maxSize =
      fieldName === "authorProfileImage" ? 10 * 1024 * 1024 : 5 * 1024 * 1024; // 10MB or 5MB
    ...
    if (file.size > maxSize) {
      ...
      console.error(
        `File too large: ${file.size} bytes (${actualSizeMB}MB), max allowed: ${maxSize} bytes (${sizeLabel})`
      );
      toast.error(
        `Image size is ${actualSizeMB}MB, but should be less than ${sizeLabel}`
      );
      return;
    }
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Lowercase, hyphen-separated (e.g.,
                enterprise-it-consulting-guide)
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
