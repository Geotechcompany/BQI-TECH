...
interface DisqualifiedTableProps {
...
export function DisqualifiedTable({ applications, jobTitles, onView, onEdit, onDelete }: DisqualifiedTableProps) {
  const columns: Column<Application>[] = [
    ...
    { 
      header: "Disqualified Date", 
      accessor: (row: Application) => {
        if (row.disqualifiedDate) {
          try {
            const date = new Date(row.disqualifiedDate);
    ...
    { 
      header: "Reason", 
      accessor: (row: Application) => row.disqualifiedReason || 'N/A'
    },
    { 
      header: "CV", 
    ...
    <div className="overflow-x-auto rounded-lg border border-border">
      {applications.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-foreground">No Disqualified Candidates</h3>
          <p className="mt-2 max-w-xl text-muted-foreground">
            Candidates who have been disqualified from the hiring process will appear here.
            Use this status to track candidates who don't meet the requirements.
          </p>
        </div>
      ) : (
